...
public class UserController {
    ...
    public ResponseEntity<?> register(@RequestBody UserRequest request) {
         ...
         * 最后调用一个end方法进行结束时间调用
         */
        LogUtils.PerformanceMonitor monitor = LogUtils.startPerformanceMonitor("USER_REGISTER");//传入的是什么事件的标志，这里的事件就是用户注册
        try {
            /**
             * 进行用户名和密码的判定为空处理
